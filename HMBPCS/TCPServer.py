from socket import *
import os  #在自动化测试中，经常需要查找操作文件，比如说查找配置文件（从而读取配置文件的信息），查找测试报告（从而发送测试报告邮件），经常要对大量文件和大量路径进行操作，这就依赖于os模块
import struct #struct模块
import json  #json模块
serverPort=12000
serverSocket=socket(AF_INET,SOCK_STREAM)  #建立socker套接字，两个参数，family代表家族地址，可为AF_INET或AF_UNIX。AF_INET家族包括Internet地址，AF_UNIX家族用于同一台机器上的进程间通信。  type代表套接字类型，可为SOCK_STREAM(流套接字)和SOCK_DGRAM(数据报套接字)，type默认TCP
...
connectionSocket,addr=serverSocket.accept()		#创建新套接字connectionSocket为用户专用，服务器套接字通过socket的accept方法等待客户请求一个连接
...
while True:
	...
	if not message:
		break
	print('There is a client ask：'+addr[0]+','+message.decode())
	if message.decode()=='q':
		connectionSocket.close()
		serverSocket.close()
		break
	elif message.decode()=='dir':
		connectionSocket.send('There are all files in the server:'.encode())
		lists=os.listdir('E:/new/')						#获取当前路径下的所有文件
		json_list=json.dumps(lists)					#用json加密
		connectionSocket.send(json_list.encode())	#把接送内容编码成字节流再发送
		...
		if buf:
			...
			while not recv_size==filesize:					#把文件内容读出来
				if filesize - recv_size > ten_percent:
					data = connectionSocket.recv(ten_percent)
					recv_size += len(data)
				else:
					data = connectionSocket.recv(filesize - recv_size)
					recv_size = filesize
