...
while True:
	...
	if message=='quit':
		...
		SocketHelper.send(connectionSocket,'There are all files in the server:')
		...
		if buf:
			...
			while not recv_size==filesize:					#把文件内容读出来
				if filesize - recv_size > ten_percent:
					...
					data = SocketHelper.recv(connectionSocket,ten_percent,None)
					recv_size += len(data)
				else:
					data = SocketHelper.recv(connectionSocket,filesize - recv_size,None)
					recv_size = filesize
